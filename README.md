...
* Frontend: https://careerhub-ashen.vercel.app
* Backend API: https://junubhire-backend.onrender.com
...
* [UI and UX Practices](#ui-and-ux-practices)
...
1. Job seekers who can register, log in, browse jobs, apply for jobs, manage their profile, build or attach resumes, reset forgotten passwords, and track their job application status.
...
* Password reset email support using Resend
...
EMAIL_FROM=JunubHire <onboarding@resend.dev>
DISABLE_AUTH=false
```

`RESEND_API_KEY` is used by the backend to send password reset emails.

`EMAIL_FROM` is the sender address used for password reset emails. During testing, Resend allows the default sender:

```env
EMAIL_FROM=JunubHire <onboarding@resend.dev>
```

For full production email delivery to any user, a verified custom domain should be added to Resend. After verifying a domain, the sender can be changed to something like:

```env
EMAIL_FROM=JunubHire <noreply@yourdomain.com>
```
...
Resume routes handle saved resume data used during job applications.
...
* Email API credentials are stored only in the backend environment file.
* Development authentication bypass must be disabled in production.
...
JunubHire is deployed using separate services for the frontend and backend.
...
```env
VITE_API_URL=https://junubhire-backend.onrender.com
VITE_DISABLE_AUTH=false
```
...
FRONTEND_URL=https://careerhub-ashen.vercel.app
RESEND_API_KEY=your_resend_api_key
EMAIL_FROM=JunubHire <onboarding@resend.dev>
DISABLE_AUTH=false
```
...
* Vercel has the correct `VITE_API_URL`
* Render has the correct `FRONTEND_URL`
* Environment variables are configured correctly
* No `.env` files are committed to GitHub
* `DISABLE_AUTH` and `VITE_DISABLE_AUTH` are not set to `true` in production
...
* Custom domain email verification for production email delivery
